...
async function fetchPendingSalons() {
    try {
        const response = await fetch('http://localhost:8080/api/v1/salons/pending', {
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
            }
...
async function authorizeSalon(salonId) {
    try {
        const response = await fetch(`http://localhost:8080/api/v1/salons/${salonId}/authorize`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'
...
async function rejectSalon(salonId) {
    try {
        const response = await fetch(`http://localhost:8080/api/v1/salons/${salonId}/reject`, {
            method: 'PUT',
            headers: {
                'Authorization': `Bearer ${token}`,
                'Content-Type': 'application/json'

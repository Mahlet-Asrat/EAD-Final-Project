document.addEventListener('DOMContentLoaded', async function() {
    ...
    try {
        // Fetch owner details
        const userResponse = await fetch('http://localhost:8080/api/v1/users/profile', {
            headers: {
        ...
        const salonResponse = await fetch('http://localhost:8080/api/v1/salons/owner', {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
...
async function handleSalonSubmission(e) {
    ...
    try {
        const response = await fetch('http://localhost:8080/api/v1/salons', {
            method: 'POST',
            headers: {
...
async function loadServices(salonId) {
    const token = localStorage.getItem('token');
    try {
        const response = await fetch(`http://localhost:8080/api/v1/salons/${salonId}/services`, {
            headers: {
                'Authorization': `Bearer ${token}`
            }
        });
